...
def get_parser():
    ...
    parser.add_argument(
        "--sample",
        metavar="14232_p1:SSC10694",
        required=True,
        help="sample:ssc_name",
        default=None,
    )
    parser.add_argument(
        "--ont_dtype",
        required=False,
        help="ONT raw data type",
        default="fast5",
        choices=["fast5", "pod5"],
    )
    parser.add_argument(
        "--proj_dir",
        help="LRA directory to the samples",
        required=True,
    )
    parser.add_argument(
        "--threads",
        help="Threads used in hard linking + generating MD5",
        required=False,
        default=8,
        type=int,
    )
    parser.add_argument(
        "--target",
        help="target data type to deposit",
        required=False,
        default="all",
        choices=["hifi", "ont", "asm", "hifiont", "hifiasm", "all"],
    )
    parser.add_argument(
        "--ont_fastq_fofn",
        help="FASTQ FOFN for ONT.",
        required=False,
        type=str,
    )
    parser.add_argument(
        "--hifi_fastq_fofn",
        help="FASTQ FOFN for HiFi.",
        required=False,
        type=str,
    )
    return parser
...
def main():
    ...
    if args.target in ["hifiont", "ont", "all"]:
        if not args.ont_fastq_fofn:
            parser.error(
                "--ont_fastq_fofn is required when --target contains 'ont'"
            )
            sys.exit(2)

        ont_obj = ONTTree(
            sample=sample_name,
            prefix=args.proj_dir,
            ssc_name=ssc_name,
            fastq_fofn=args.ont_fastq_fofn,
        )
        ont_df = ont_obj.desired_tree_df()

        distribute_hard_link_jobs(
            src_list=ont_df["src_file"],
            dest_list=ont_df["dest_file"],
            threads=threads,
        )
        ...
        MD5Generator(
            search_dict=ont_search_dict,
            num_workers=threads,
        ).write_md5sums()

        ont_bc_dest_md5_df = get_reads_dest_md5_df(
            reads_df=ont_bc_df,
            search_dict=ont_search_dict,
        )
        for md5_index in range(len(ont_bc_dest_md5_df)):
            if not os.path.exists(ont_bc_dest_md5_df.dest_file[md5_index]):
                file_df = pd.read_table(
                    ont_bc_dest_md5_df.src_file[md5_index],
                    names=["md5", "file_name"],
                ).sort_values(by="file_name")
                file_df["file_name"] = file_df.file_name.str.replace(
                    sample_name,
                    ssc_name,
                )
                file_df.to_csv(
                    ont_bc_dest_md5_df.dest_file[md5_index],
                    sep="\t",
                    index=False,
                    header=False,
                )
    ...
    if args.target in ["hifiont", "hifi", "all"]:
        if not args.hifi_fastq_fofn:
            parser.error(
                "--hifi_fastq_fofn is required when --target contains 'hifi'"
            )
            sys.exit(2)
        ...
        hifi_obj = HiFiTree(
            ...
            prefix=args.proj_dir,
            ssc_name=ssc_name,
            fastq_fofn=args.hifi_fastq_fofn,
        )
        hifi_df = hifi_obj.desired_tree_df()

        distribute_hard_link_jobs(
            src_list=hifi_df["src_file"],
            dest_list=hifi_df["dest_file"],
            threads=threads,
        )
        ...
        MD5Generator(
            search_dict=hifi_search_dict,
            num_workers=threads,
        ).write_md5sums()

        hifi_dest_md5_df = get_reads_dest_md5_df(
            reads_df=hifi_df,
            search_dict=hifi_search_dict,
        )
        for md5_index in range(len(hifi_dest_md5_df)):
            if not os.path.exists(hifi_dest_md5_df.dest_file[md5_index]):
                file_df = pd.read_table(
                    hifi_dest_md5_df.src_file[md5_index],
                    names=["md5", "file_name"],
                ).sort_values(by="file_name")
                file_df["file_name"] = file_df.file_name.str.replace(
                    sample_name,
                ...
                file_df.to_csv(
                    hifi_dest_md5_df.dest_file[md5_index],
                    sep="\t",
                    index=False,
                    header=False,
                )
    ...
    if args.target in ["hifiasm", "asm", "all"]:
        ...
        if trio_exists:
            asm_df = asm_df[
                ~asm_df.dest_dir.str.contains("hifiasm/\\d+.\\d+.\\d+", regex=True)
            ]

        distribute_hard_link_jobs(
            src_list=asm_df["src_file"],
            dest_list=asm_df["dest_file"],
            threads=threads,
        )
        ...
        asm_dest_md5_df = get_asm_dest_md5_df(
            asm_df=asm_df,
            search_dict=asm_search_dict,
        )

        hifiasm_df = asm_dest_md5_df[
            asm_dest_md5_df.dest_file.str.contains("hifiasm")
        ]
        if not os.path.exists(hifiasm_df.dest_file.iloc[0]):
            file_df = pd.read_table(
                hifiasm_df.src_file.iloc[0],
                names=["md5", "file_name"],
            )
            file_df["file_name"] = file_df.file_name.str.replace(
                sample_name,
            ...
            asm_dir = "/".join(hifiasm_df.dest_file.iloc[0].split("/")[:-1])
            if not os.path.exists(asm_dir):
                os.makedirs(asm_dir, exist_ok=True)
            file_df.to_csv(
                hifiasm_df.dest_file.iloc[0],
                sep="\t",
                index=False,
                header=False,
            )
            LOG.info(f"wrote: {hifiasm_df.dest_file.iloc[0]}")
        else:
            LOG.info(f"{hifiasm_df.dest_file.iloc[0]} exists, ignoring.")

        distribute_hard_link_jobs(
            src_list=hifiasm_df["src_file"],
            dest_list=hifiasm_df["dest_file"],
            threads=threads,
        )
...
def distribute_hard_link_jobs(src_list, dest_list, threads=8):
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [
            executor.submit(make_dir_and_hard_link, src, dest)
...
def build_local_md5_cache_path(src_dir, md5_name):
    src_dir = os.path.abspath(src_dir)
    rel_dir = src_dir.lstrip(os.sep)
    return os.path.join(".", ".md5_cache", rel_dir, md5_name)
...
def get_reads_dest_md5_df(reads_df, search_dict):
    ...
    for row in reads_df.itertuples():
        if "fastq.gz" in row.dest_file:
            dest_md5_files.append(os.path.join(row.dest_dir, "fastq.md5"))
        elif ".bam" in row.dest_file:
            dest_md5_files.append(os.path.join(row.dest_dir, "bam.md5"))
        else:
            raise ValueError(f"Reads md5 not accnted for: {row.src_file}")
    dest_md5_files = set(dest_md5_files)

    for idx, key in enumerate(search_dict.keys()):
        md5_basename = os.path.basename(key)
        dest_md5 = [f for f in dest_md5_files if f.endswith(md5_basename)][0]
        dest_md5_df.loc[idx, :] = key, dest_md5
...
def get_asm_dest_md5_df(asm_df, search_dict):
    ...
    for row in asm_df.itertuples():
        if ".fasta" in row.dest_file:
            dest_md5_files.append(os.path.join(row.dest_dir, "fasta.md5"))
        else:
    ...
    def fetch_md5sum(self):
        def get_rerun_flag(search_dict):
            ...
            for fofn in search_dict.keys():
                ...
                fofn_df["fname"] = fofn_df["fname"].str.replace(
                    "^./",
                    "",
                    regex=True,
                )
                fofn_df["base_name"] = fofn_df["fname"].apply(os.path.basename)
                fofn_df["base_name"] = fofn_df["base_name"].str.replace(
                    "^./",
                    "",
                    regex=True,
                )
                ...
                for base_name in base_names:
                    if base_name not in fofn_df_base_names:
                        rerun_flag += 1
            return rerun_flag
        ...
        for key in self.search_dict.keys():
            values = []
            if isinstance(self.search_dict[key], list):
                search_space = self.search_dict[key]
            else:
                search_space = [self.search_dict[key]]
            ...
            if rerun_flag > 0:
                for each_file in search_space:
                    ...
                    values.append(self.make_md5sum(file_name=each_file))
                    LOG.info(f"md5 fetched: {each_file}")
            else:
                LOG.info("md5 exists, skipping calculation.")
                exist_md5sum_df = (
                    pd.read_csv(key, sep="\t", header=None)
                    .drop_duplicates()
                    .sort_values(by=1)
                )
                exist_md5sum_df[1] = exist_md5sum_df[1].str.replace(
                    "^./",
                    "",
                    regex=True,
                )
                values = list(exist_md5sum_df.itertuples(index=False, name=None))

            md5sums_dict[key] = values

        return md5sums_dict

    def write_md5sums(self):
        md5sums_dict = self.fetch_md5sum()
        for key in md5sums_dict.keys():
            os.makedirs(os.path.dirname(key), exist_ok=True)
            with open(key, "w") as outfile:
                for md5, file_name in md5sums_dict[key]:
                    outfile.write(f"{md5}\t./{file_name}\n")
            LOG.info(f"file wrote: {key}")
...
class ASMTree:
    ...
    def get_assemblies(self) -> list:
        ...
        if not is_empty:
            assembly_file_list = [
                x.as_posix()
                for x in list(path_obj.rglob("*.fasta*"))
                if not x.is_symlink()
            ]
            assembly_file_list = [
                x for x in assembly_file_list if "fix_sex_chr" not in x
            ]
        else:
            assembly_file_list = []
    ...
    def desired_tree_df(self) -> pd.DataFrame:
        df = pd.DataFrame(data={"src_file": self.get_assemblies})
        ...
        for entry in df.itertuples():
            ...
            if "hifiasm" in src_fn:
                placeholder_suffix = (
                    f"./fastq_assembly/{self.ssc_name}/genome_assembly/{target_suffix}"
                )
                parental_origin = {"dip.hap1": "dip.pat", "dip.hap2": "dip.mat"}
                for old, new in parental_origin.items():
                    placeholder_suffix = placeholder_suffix.replace(old, new)

                placeholder_suffix = placeholder_suffix.replace(
                    self.sample,
                    self.ssc_name,
                )
                df.loc[entry.Index, "dest_file"] = placeholder_suffix
            else:
                raise ValueError(src_fn)
    ...
    def find_md5(df):
        ...
        for src_dir, file_list in md5_dict.items():
            if "hifiasm" in src_dir:
                new_key = build_local_md5_cache_path(src_dir, "fasta.md5")
                new_md5_dict[new_key] = file_list

        return new_md5_dict


class HiFiTree:
    def __init__(self, sample, prefix, ssc_name, fastq_fofn):
        self.sample = sample
        self.prefix = prefix
        self.ssc_name = ssc_name
    ...
    def get_hifi_raw_data(self, file_type="bam") -> list:
        ...
        if file_type == "bam":
            return self.get_bams(path_obj=path_obj, which_one="hifi")
        if file_type == "fastq":
            return self.get_fastq_gz(path_obj=path_obj)
        raise ValueError(f"Invalid argument for file_type: {file_type}")

    def _validate_selected_hifi_files(self, available_files):
        ...
        for fastq in selected_fastq_list:
            bam = re.sub(r"\.fastq\.gz$", ".bam", fastq)
            required_files = [
                fastq,
                bam,
            ]
            missing = [fp for fp in required_files if fp not in available_set]
            if missing:
                missing_records.append({"fastq": fastq, "missing": missing})

        if missing_records:
            error_lines = [
                "Missing HiFi required files detected for entries in --hifi_fastq_fofn:"
            ]
            for record in missing_records:
                error_lines.append(f"- {record['fastq']}")
                for missing_file in record["missing"]:
                    error_lines.append(f"    missing: {missing_file}")
        ...
        for fastq in selected_fastq_list:
            ...
            selected_files.append(fastq)
            selected_files.append(bam)

            for optional_file in optional_files:
                if optional_file in available_set:
                    selected_files.append(optional_file)

        return selected_files

    def _selected_hifi_files(self):
        all_files = (
            self.get_hifi_raw_data(file_type="fastq")
            + self.get_hifi_raw_data(file_type="bam")
        )
        return self._validate_selected_hifi_files(available_files=all_files)
    ...
    def desired_tree_df(self) -> pd.DataFrame:
        ...
        for entry in df.itertuples():
            ...
            if "fastq.gz" in src_fn:
                df.loc[entry.Index, "dest_file"] = (
                    f"./fastq_assembly/{self.ssc_name}/{target_suffix}"
                )
            elif ".bam" in src_fn:
                df.loc[entry.Index, "dest_file"] = (
                    f"./raw_data/{self.ssc_name}/{target_suffix}"
                )
            else:
                raise ValueError(src_fn)
    ...
    def find_md5(df):
        ...
        for src_dir, file_list in md5_dict.items():
            bam_files = [x for x in file_list if ".bam" in x]
            fastq_files = [x for x in file_list if "fastq" in x]

            if bam_files:
                new_key = build_local_md5_cache_path(src_dir, "bam.md5")
                new_md5_dict[new_key] = bam_files

            if fastq_files:
                new_key = build_local_md5_cache_path(src_dir, "fastq.md5")
                new_md5_dict[new_key] = fastq_files
...
class ONTTree:
    ...
    def get_data(self):
        def sort_cr_list(copy_record_list):
            sorted_copy_record_list = [
                copy_record for copy_record in copy_record_list
                if "/pod5/" in copy_record
            ]
            pod5_run_ids = [
                copy_record.split("/")[-1].split(".")[0]
                for copy_record in sorted_copy_record_list
            ]
            fast5_copy_record_list = [
                copy_record for copy_record in copy_record_list
                if "/fast5/" in copy_record
            ]

            for copy_record in fast5_copy_record_list:
                run_id = copy_record.split("/")[-1].split(".")[0]
                if run_id not in pod5_run_ids:
        ...
        df = pd.DataFrame()
        if not is_empty:
            search_for_cr = list(path_obj.rglob("copy_record/*/*.tab.gz"))
            if search_for_cr:
                ...
                df = pd.concat(
                    [
                        pd.read_table(
                            x,
                            usecols=["RUN_ID", "DEST_PATH", "SIZE", "MD5"],
                            dtype="object",
                        )
                        for x in copy_record_list
                    ]
                ).reset_index(drop=True)

                for fp in copy_record_list:
                    check_df = pd.read_table(
                        fp,
                        usecols=["RUN_ID", "DEST_PATH", "SIZE", "MD5"],
                        dtype="object",
                    )
                    for row in check_df.itertuples():
                        search_path = os.path.join(self.prefix, row.DEST_PATH)
                        if not os.path.exists(search_path):
                            raise OSError(f"{search_path} doesn't exist.")
            else:
                raise OSError("No copy record found.")
        ...
        for entry in df.itertuples():
            ...
            df.loc[entry.Index, "BASECALLED_DATA_PREFIX"] = os.path.join(
                self.prefix,
                self.sample,
                *path_split[target_idx:target_idx + 3],
                "fastq",
                entry.RUN_ID,
            )
    ...
    def find_ont_md5(df):
        ...
        for src_dir, basecalled_files in md5_dict.items():
            fastq_files = []
            bam_files = []
            for bc_file in basecalled_files:
                if re.search("fastq.gz", bc_file):
                    fastq_files.append(bc_file)
                elif re.search(r"\.bam(\.bai)?$", bc_file):
                    bam_files.append(bc_file)

            if fastq_files:
                fastq_key = build_local_md5_cache_path(src_dir, "fastq.md5")
                new_md5_dict[fastq_key] = fastq_files

            if bam_files:
                bam_key = build_local_md5_cache_path(src_dir, "bam.md5")
                new_md5_dict[bam_key] = bam_files

        return new_md5_dict

    def _validate_selected_ont_files(self, available_files):
        selected_fastq_list = pd.read_csv(self.fastq_fofn, header=None)[0].tolist()
        available_set = set(available_files)
        missing_records = []
        ...
        if missing_records:
            error_lines = [
                "Missing ONT required files detected for entries in --ont_fastq_fofn:"
            ]
            for record in missing_records:
                error_lines.append(f"- {record['fastq']}")
                for missing_file in record["missing"]:
    ...
    def get_basecalled_data(self, df):
        ...
        passed_basecalled_data_list = self._validate_selected_ont_files(
            available_files=bc_df["src_file"].tolist()
        )

        bc_df = bc_df[bc_df.src_file.isin(passed_basecalled_data_list)]
        bc_df["src_dir"] = bc_df["src_file"].apply(os.path.dirname)

        for entry in bc_df.itertuples():
            src_fn = entry.src_file

            if re.search(r"\.bam(\.bai)?$", src_fn):
                target_suffix = src_fn[src_fn.index("fastq"):]
                target_suffix = self.manipulate_filename(target_suffix)
                bc_df.loc[entry.Index, "dest_file"] = os.path.join(
                    ".",
                    "raw_data",
                    self.ssc_name,
                    "nanopore",
                    self.library,
                    target_suffix.replace("fastq", "bam"),
                )
            else:
    ...
    def desired_tree_df(self):
        ...
        for entry in df.itertuples():
            src_fn = entry.src_file
            target_suffix = src_fn[src_fn.index("nanopore"):]
            df.loc[entry.Index, "dest_file"] = os.path.join(
                ".",
                "raw_data",
                self.ssc_name,
                target_suffix,
        ...
        self.raw_data_md5_df = (
            df.groupby(["dest_outfile_name"], group_keys=True)[["src_basename", "MD5"]]
        ...
        for entry in self.raw_data_md5_df.itertuples():
            ...
            if not os.path.isfile(outfn):
                ...
                with open(outfn, "a") as outfile:
                    for md5sum, basename in entry.list_of_tuples:
